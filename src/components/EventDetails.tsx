...
function DetailCard({ symbol, title, lines }: DetailCardProps) {
  return (
    ...
    >
      ...
      <div className="h-px w-10" style={{ backgroundColor: 'rgba(196,113,74,0.4)' }} />
      {lines.map((line, i) => (
        <p key={i} className="font-sans text-sm leading-relaxed" style={{ color: 'rgba(44,36,22,0.65)' }}>
          {line}
        </p>
      ))}
    </div>
...
export default function EventDetails() {
  ...
  return (
    <section ref={sectionRef} className="py-24 px-6" style={{ backgroundColor: '#F5EDE3' }}>
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <DetailCard
            symbol="◈"
            title="Hora"
            lines={['Ceremonia', '5:00 PM', 'Recepción 7:00 PM']}
          />
          <DetailCard
            symbol="◈"
            title="Lugar"
            lines={['Nombre del Venue', 'Dirección completa', 'Ciudad, País']}
          />
          <DetailCard
            symbol="◈"
            title="Vestimenta"
            lines={['Formal', 'Paleta otoñal preferida', 'Sin blanco']}
          />
        </div>
        ...
        <div className="text-center mt-20">
          <div className="flex items-center justify-center gap-6 mb-8">
            <div className="h-px flex-1 max-w-20" style={{ backgroundColor: 'rgba(196,113,74,0.3)' }} />
            <p
              className="font-serif italic font-light"
              style={{ color: 'rgba(44,36,22,0.55)', fontSize: 'clamp(1rem, 2.5vw, 1.35rem)' }}
            >
